...
def load_pipeline(args, device, dtype):
    ...
    try:
        ...
        # 自动获取 adapter 名称（SD3.5 必做）
        adapter_names = pipeline.get_active_adapters()
        print(f"🔎 Available LoRA adapters: {adapter_names}")

        pipeline.set_adapters(
            adapter_names,
            adapter_weights=[args.lora_scale] * len(adapter_names)
        )

        print(f"✅ LoRA adapters activated with scale={args.lora_scale}")
